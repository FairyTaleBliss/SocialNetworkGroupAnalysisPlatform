...
# SIR模拟传播
def SIRsimulation(rateSI, rateIR, Sus, Inf, Res ):
    edges_set = []
    edgenum = EdgeNumber()  # 边的序号
    N = NeighborNodeSet()  # 邻居节点集合
    # 对所有的已经感染的节点，以rateIR的概率恢复，将不会再被感染
    for n in Inf:
        if random.random() <= rateIR:
            Res.append(n)  # 已恢复节点列表
            Inf.remove(n)
    # 对感染节点的邻居，以rateSI的概率被感染
    for i in range(len(Inf)):
        s = int(Inf[i])
        for j in N[s - 1]:
            if (j in Sus and j not in Res):
                if random.random() <= rateSI:
                    Inf.append(j)
                    Sus.remove(j)
                    edges_set.append(edgenum[s-1][j-1])
    return Sus, Inf, Res, edges_set
